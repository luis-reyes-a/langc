...
EatToken(lexer_state *lexer)
{
    ...
    switch(*lexer->at)
    {
        ...
        case '#': 
        {
            if(lexer->at[1] == '#' && lexer->at[2] == '#')
            {
                result.type = TokenType_UnInitialized;
                lexer->at += 3;
            }
            else
            {
                result.type = *lexer->at;
                ++lexer->at;
            }
        }
        break;
        ...
        {
            string_array_search_result search = IndexInStringArray(lexer->at, global_keywords, array_count(global_keywords));
            if(search.length)
            {
...
internal int
eats_token(lexer_state *lexer, lexer_token_type type)
{
...
internal int
eats_keyword(lexer_state *lexer, keyword_type keyword)
{
...
internal void
expect_token(lexer_state *lexer, lexer_token_type type)
{
    if(!eats_token(lexer, type))
    {
...
internal void
expect_keyword(lexer_state *lexer, keyword_type keyword)
{
    if(!eats_keyword(lexer, keyword))
    {
...
internal char *
expect_identifier(lexer_state *lexer)
{
    char *identifier = 0;
