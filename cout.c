...
OutputTypeSpecifier(writeable_text_buffer *buffer, type_specifier *typespec, char wrapper)
{
    ...
    switch(typespec->type)
    {
        ...
        case TypeSpec_Array:   
        {
            OutputTypeSpecifier(buffer, typespec->base_type ,0); 
    ...
    switch(expr->type)
    {
        ...
        case Expression_Compound:   
        {
                ...
                member = member->member_next)
            {
                ...
                OutputExpression(buffer, member->member_expression, 0);
...
PrefixVariablesWithStructIfNecessary(writeable_text_buffer *buffer, declaration *decl, type_specifier *ref_type)
{
    ...
    {
        if((decl->typespec->type == TypeSpec_Struct || 
            decl->typespec->type == TypeSpec_Union) &&
           decl->typespec == ref_type)
        {
            panic(); //Infinte nesting
        }
        else if(decl->typespec->type == TypeSpec_Array && 
                decl->typespec->base_type == ref_type)
        {
            panic(); //Infinte nesting
        }
        else if (decl->typespec->type == TypeSpec_Ptr && decl->typespec->base_type == ref_type)
        {
    ...
    switch(decl->type)
    {
        ...
        case Declaration_ProcedureArgs:   
        {
            ...
            {//TODO is this correct order in C?
                OutputChar(buffer, '[', 0);
                OutputInteger64(buffer, top->size); //TODO expression
                OutputChar(buffer, ']', 0);
            }
        ...
        case Declaration_Constant:
        {
            ...
            OutputExpression(buffer, decl->initializer_as_const, 0);
            OutputChar(buffer, '\n', 0);
        }break;
        ...
        if(decl->identifier)
        {
            this_type_spec = find_type(decl->identifier);
        }
        ...
        case Declaration_ProcedureGroup: //NOTE just prints out foward decls
        {
            ...
            for(declaration *sign = decl->members;
                ...
                sign= sign->next)
            {
                if(sign->keyword == Keyword_Internal)  
                    OutputCString(buffer, "static ", 0);
                else if(sign->keyword == Keyword_External)  
                    OutputCString(buffer, "extern ", 0);
                else if(sign->keyword == Keyword_Inline)  
                    OutputCString(buffer, "inline ", 0); //TODO compiler specific force inline
                else if(sign->keyword == Keyword_NoInline)  ;
                //TODO compiler specific force no_inline
                else panic();
                OutputTypeSpecifier(buffer, sign->return_type, ' ');
                OutputCString(buffer, sign->identifier, 0);
                OutputChar(buffer, '(', 0);
        ...
        case Declaration_Procedure:   
        {
            ...
            if(decl->keyword == Keyword_Internal)  
                OutputCString(buffer, "static ", 0);
            else if(decl->keyword == Keyword_External)  
                OutputCString(buffer, "extern ", 0);
            else if(decl->keyword == Keyword_Inline)  
                OutputCString(buffer, "inline ", 0); //TODO compiler specific force inline
            else if(decl->keyword == Keyword_NoInline)  ;
            //TODO compiler specific force no_inline
            else panic();
            OutputTypeSpecifier(buffer, decl->return_type, ' ');
            OutputCString(buffer, decl->identifier, 0);
            OutputChar(buffer, '(', 0);
